...
@PIPELINES.register_module()
class LoadImageFromFileMultiDataset(LoadImageFromFile):
    """Load an image from file.
    ...
    def __call__(self, results):
        ...
        super(LoadImageFromFileMultiDataset, self).__call__(results)
        results['clstype'] = results['img_info']['clstype']
        return results
...
class LoadImage(object):
    ...
    def _load_imgs(self, results):
        ...
        :return:
        """
        img_bytes = self.file_client.get(results["img_meta"]["img_path"])
        img = mmcv.imfrombytes(img_bytes, flag=self.color_type)
        ...
        if "ori_shape" not in results["img_meta"].keys():
            results["img_meta"]["ori_shape"] = img.shape

        if "img_shape" not in results["img_meta"].keys():
            results["img_meta"]["img_shape"] = img.shape

        # if "scale_factor" not in results["img_meta"].keys():
        #     results["img_meta"]["scale_factor"] = [img.shape[1], img.shape[0]]
    ...
    def __call__(self, results):
        ...
        """
        results = self._load_imgs(results)
